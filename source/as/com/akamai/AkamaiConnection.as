...
package com.akamai
{
	...
	public class AkamaiConnection extends EventDispatcher
	{
		...
		{
//			trace( "AkamaiConnection.dispose()" );
			close();
			
			if ( _akLoader )
			{
				_akLoader.removeEventListener("complete", akamaiXMLLoaded);
				_akLoader.removeEventListener(IOErrorEvent.IO_ERROR, catchIOError);
			}
			
			if ( _nsId3) 
			{
				_nsId3.removeEventListener(Event.ID3,onId3);
				_nsId3.removeEventListener(NetStatusEvent.NET_STATUS,id3StreamStatus);
			}
			
			if ( _ns )
			{
				_ns.removeEventListener(NetStatusEvent.NET_STATUS,streamStatus);
				_ns.removeEventListener(AsyncErrorEvent.ASYNC_ERROR,asyncError);
				_ns.removeEventListener(IOErrorEvent.IO_ERROR,netStreamIOError);
			}
			if ( _nc )
			{
				_nc.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,netSecurityError);
				_nc.removeEventListener(AsyncErrorEvent.ASYNC_ERROR,asyncError);
			} 
			...
			_liveStreamTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, liveStreamTimeout);
			_liveFCSubscribeTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, liveFCSubscribeTimeout);
			_liveStreamRetryTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, retrySubscription);
			_progressTimer.removeEventListener(TimerEvent.TIMER, updateProgress);
			_bufferFailureTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, handleBufferFailure);
			
			
			if ( _aNC )
			{
				var ln : int = _aNC.length;
				for ( var i:Number=0; i < ln; i++ ) 
				{
					if ( _aNC[i] != null )
					{
						_aNC[i].removeEventListener(NetStatusEvent.NET_STATUS,netStatus);
			    		_aNC[i].removeEventListener(SecurityErrorEvent.SECURITY_ERROR,netSecurityError);
			    		_aNC[i].removeEventListener(AsyncErrorEvent.ASYNC_ERROR,asyncError);
						_aNC[i].removeEventListener(AkamaiNotificationEvent.BANDWIDTH,bubbleNotificationEvent);
						_aNC[i].removeEventListener(AkamaiStatusEvent.FCSUBSCRIBE,onFCSubscribe);
						_aNC[i].removeEventListener(AkamaiStatusEvent.FCUNSUBSCRIBE,onFCUnsubscribe);
					}
				}
			}
//			Log.debug( "[AkamaiConnection] dispose() " );
		}
    	...
    	/** Catches netstream XMP Data events
    	 * @private
    	 */
		public function onXMPData(info:Object):void {
        		dispatchEvent(new AkamaiStatusEvent(AkamaiStatusEvent.NETSTREAM_XMP_DATA, info ) );
    	}
